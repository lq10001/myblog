package com.ly.sys.action;

import com.ly.comm.Bjui;
import com.ly.comm.Page;
import com.ly.comm.ParseObj;
import org.nutz.dao.Cnd;
import org.nutz.dao.Dao;
import org.nutz.ioc.loader.annotation.Inject;
import org.nutz.ioc.loader.annotation.IocBean;
import org.nutz.log.Log;
import org.nutz.log.Logs;
import org.nutz.mvc.annotation.*;
import org.nutz.mvc.filter.CheckSession;

import javax.servlet.http.HttpServletRequest;
import java.util.List;
import java.util.Map;

import net.sf.ehcache.CacheManager;


import com.ly.sys.vo.Menu;
import com.ly.sys.service.MenuService;


@IocBean
@At("/menu")
@Fail("json")
@Filters(@By(type=CheckSession.class, args={"username", "/WEB-INF/login.html"}))
public class MenuAction {

	private static final Log log = Logs.getLog(MenuAction.class);
	
	@Inject
	private MenuService menuService;

    @At("/")
    @Ok("beetl:/WEB-INF/sys/menu_list.html")
    public void index(@Param("..")Page p,
                      @Param("..")Menu menu,
                      HttpServletRequest request){

        Cnd c = new ParseObj(menu).getCnd();
        if (c == null || c.equals(""))
        {
            p.setRecordCount(menuService.listCount(c));
            request.setAttribute("list_obj", menuService.queryCache(c,p));
        }else{
            p.setRecordCount(menuService.count(c));
            request.setAttribute("list_obj", menuService.query(c,p));
        }

        request.setAttribute("page", p);
        request.setAttribute("menu", menu);
    }

    @At
    @Ok("beetl:/WEB-INF/sys/menu.html")
    public void edit(@Param("action")int action,
                     @Param("id")Long id,
                      HttpServletRequest request){
        if(id == null || id == 0){
            request.setAttribute("menu", null);
        }else{

            Menu menu = menuService.fetch(id);
            request.setAttribute("menu", menu);
        }
        request.setAttribute("action", action);
    }

    @At
    @Ok("json")
    public Map<String,String> save(@Param("action")int action,
                                @Param("..")Menu menu){
        Object rtnObject;
        if (menu.getId() == null || menu.getId() == 0) {
            rtnObject = menuService.dao().insert(menu);
        }else{
            if (action == 3) {
                menu.setId(null);
                rtnObject = menuService.dao().insert(menu);
            }else{
                rtnObject = menuService.dao().updateIgnoreNull(menu);
            }
        }
        CacheManager.getInstance().getCache(MenuService.CACHE_NAME).removeAll();
        return Bjui.rtnMap((rtnObject == null) ? false : true, "tab_menu", true);

    }

    @At
    @Ok("json")
    public Map<String,String> del(@Param("id")Long id)
    {
        int num =  menuService.delete(id);
        CacheManager.getInstance().getCache(MenuService.CACHE_NAME).removeAll();
        return Bjui.rtnMap((num > 0) ? true : false , "tab_menu",false);
    }

}
